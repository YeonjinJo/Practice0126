...
function App() {
  ...
  const filterByAge = (minAge) => {
    const filteredList = students.filter((element) => element.age >= minAge);
    setFilteredStudents(filteredList);
  };

  // TODO: filterByGrade 함수를 작성하세요. 이 함수는 특정 학점을 매개변수로 받아 해당 학점의 학생들로 필터링해야 합니다.
  const filterByGrade = (grade) => {
    const filteredList = students.filter((element) => element.grade === grade);
    setFilteredStudents(filteredList);
  };
  ...
  return (
    <div>
      ...
      <input
        ...
        type="number"
      />
      <button onClick={() => filterByAge(minAge)}>세 이상</button>
